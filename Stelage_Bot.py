...
# old_TOKEN = '5535925275:AAFxpOKMb8Mwdkgs3biExSWYZN48OhEMhkU'

# --- new TOKEN ---
TOKEN = '2068653895:AAFDkwyO59u1-iDkIryXuua3rFpA-bJzmZo'
...
keyboard_main.row('Новости', '️Корзина')
keyboard_main.row('Контакты и Связь с Менедежером', 'Помощь (FAQ)')
...
keyboard_admin.row('Стеллажи')
keyboard_admin.row('Выгрузка товаров')
...
def load(message):
    ...
    elif message.text == 'Выгрузка товаров':
...
def shelf_step(message):
    try:
        ...
        orderList = [model.mod, model.height, model.width, model.depth, model.shelf]
        ...
        data = modelStelage.drop(columns=['Производитель', 'Вес', 'Сборка', 'Грузоподъёмность', 'Назначение', 'id'],
                                 axis=1)
        ...
        bot.send_message(message.chat.id, 'Ошибка! Пожалуйста, перезапустите бота (команда /start)' + str(e))
...
def text_handler(message):
    ...
    if message.text == 'В главное меню!':
        ...
        bot.send_message(message.chat.id, 'Новости!', reply_markup=keyboard_main)

    elif message.text == 'Помощь (FAQ)':
        bot.send_message(message.chat.id, '*Как мы работаем?*\n\n'
                                          '❕ *Оформляете* заказ\n'
                                          '❕ *Специалист связывается* и утверждает детали заказа\n'
                                          '❕ *Мы доставляем заказ* по указанному адресу\n'
        ...
        msg = bot.send_message(message.chat.id, 'Напишите Ваш вопрос!', reply_markup=keyboard_question)
        bot.register_next_step_handler(msg, question)

    elif message.text == 'Новый заказ':
        bot.send_message(message.chat.id,
                         'Нажмите для создания нового заказа /calculator',
                         reply_markup=keyboard_main)
...
def excel_handler(message):
    try:
        ...
        bot.send_message(message.chat.id, 'Что-то пошло не так, попробуйте ещё!' + str(e))
        bot.register_next_step_handler_by_chat_id(message.chat.id, load)
